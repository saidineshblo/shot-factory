...
export class Versions extends APIResource {
   ...
   * @example
   * ```ts
   * await replicate.models.versions.list({
  ...
  list(params: VersionListParams, options?: RequestOptions): APIPromise<void> {
    const { model_owner, model_name } = params;
    return this._client.get(path`/models/${model_owner}/${model_name}/versions`, {
      ...options,
   ...
   * @example
   * ```ts
   * await replicate.models.versions.delete({
   *   model_owner: 'model_owner',
   *   model_name: 'model_name',
   ...
   * ```
   */
  delete(params: VersionDeleteParams, options?: RequestOptions): APIPromise<void> {
    const { model_owner, model_name, version_id } = params;
    return this._client.delete(path`/models/${model_owner}/${model_name}/versions/${version_id}`, {
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
  }
   ...
   * @example
   * ```ts
   * await replicate.models.versions.get({
   *   model_owner: 'model_owner',
   *   model_name: 'model_name',
   *   version_id: 'version_id',
   * });
   * ```
   */
  get(params: VersionGetParams, options?: RequestOptions): APIPromise<void> {
    const { model_owner, model_name, version_id } = params;
    return this._client.get(path`/models/${model_owner}/${model_name}/versions/${version_id}`, {
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
  }
