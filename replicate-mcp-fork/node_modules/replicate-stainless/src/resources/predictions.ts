...
import {
  CursorURLPageWithCreatedFilters,
...
export class Predictions extends APIResource {
   ...
   */
  create(params: PredictionCreateParamsWithVersion, options?: RequestOptions): APIPromise<Prediction> {
    const { Prefer, ...body } = params;
    return this._client.post('/predictions', {
      body,
      ...options,
   ...
   */
  list(
    query: PredictionListParams | null | undefined = {},
    options?: RequestOptions,
  ): PagePromise<PredictionsCursorURLPageWithCreatedFilters, Prediction> {
    return this._client.getAPIList('/predictions', CursorURLPageWithCreatedFilters<Prediction>, {
      query,
      ...options,
    });
  }
   ...
   * ```
   */
  cancel(params: PredictionCancelParams, options?: RequestOptions): APIPromise<Prediction> {
    const { prediction_id } = params;
    return this._client.post(path`/predictions/${prediction_id}/cancel`, options);
  }
   ...
   */
  get(params: PredictionGetParams, options?: RequestOptions): APIPromise<Prediction> {
    const { prediction_id } = params;
    return this._client.get(path`/predictions/${prediction_id}`, options);
  }
}

export type PredictionsCursorURLPageWithCreatedFilters = CursorURLPageWithCreatedFilters<Prediction>;
 ...
 * the model
 */
export type PredictionOutput = unknown;

export interface PredictionRequest {
  /**
   * The model's input as a JSON object. The input schema depends on what model you
  ...
  /**
   * An HTTPS URL for receiving a webhook when the prediction has new output. The
...
export interface PredictionCreateParams {
  /**
   * Body param: The model's input as a JSON object. The input schema depends on what
   * model you are running. To see the available inputs, click the "API" tab on the
   * model you are running or [get the model version](#models.versions.get) and look
   * at its `openapi_schema` property. For example,
   * [stability-ai/sdxl](https://replicate.com/stability-ai/sdxl) takes `prompt` as
   * an input.
   *
   * Files should be passed as HTTP URLs or data URLs.
  ...
  /**
   * Body param: An HTTPS URL for receiving a webhook when the prediction has new
   * output. The webhook will be a POST request where the request body is the same as
   * the response body of the [get prediction](#predictions.get) operation. If there
   * are network problems, we will retry the webhook a few times, so make sure it can
   * be safely called more than once. Replicate will not follow redirects when
   * sending webhook requests to your service, so be sure to specify a URL that will
   * resolve without redirecting.
   */
  webhook?: string;

  /**
   * Body param: By default, we will send requests to your webhook URL whenever there
   * are new outputs or the prediction has finished. You can change which events
   * trigger webhook requests by specifying `webhook_events_filter` in the prediction
   * request:
   *
   * - `start`: immediately on prediction start
   * - `output`: each time a prediction generates an output (note that predictions
  ...
  /**
   * Header param: Leave the request open and wait for the model to finish generating
   * output. Set to `wait=n` where n is a number of seconds between 1 and 60.
   *
   * See https://replicate.com/docs/topics/predictions/create-a-prediction#sync-mode
   * for more information.
   */
  Prefer?: string;
}

export interface PredictionCreateParamsWithVersion extends PredictionCreateParams {
  /**
   * Body param: The ID of the model version that you want to run. This can be
...
export interface PredictionListParams extends CursorURLPageWithCreatedFiltersParams {}
